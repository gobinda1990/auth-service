...
//
//    private final ConcurrentHashMap<String, CustomUserDetails> tokenUserCache = new ConcurrentHashMap<>();
//
//    @Override
...
//
//            if (accessTokenOpt.isPresent() && jwtService.validateToken(accessTokenOpt.get())) {
//                authenticateUser(accessTokenOpt.get());
//            } else if (refreshTokenOpt.isPresent() && jwtService.validateToken(refreshTokenOpt.get())) {
//                handleRefreshToken(refreshTokenOpt.get(), response);
//            }
//        } catch (Exception ex) {
...
//        jwtService.extractUsername(refreshToken).ifPresent(username -> {
//            CustomUserDetails userDetails = tokenUserCache.computeIfAbsent(refreshToken,
//                    t -> (CustomUserDetails) authServiceDetails.loadUserByUsername(username));
//
//            String newAccessToken = jwtService.generateAccessToken(userDetails);
//
//            // Only set access_token in cookie if needed
...
//            log.debug("Issued new access token for user {}", username);
//        });
//    }
//
...
//                CustomUserDetails userDetails = tokenUserCache.computeIfAbsent(token,
//                        t -> (CustomUserDetails) authServiceDetails.loadUserByUsername(username));
//
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
                ...
                if (jwtService.validateToken(refreshToken)) {
                    ...
                    if (username != null) {
                        CustomUserDetails userDetails =
                                (CustomUserDetails) authServiceDetails.loadUserByUsername(username);
    ...
    private void sendJsonError(HttpServletResponse response, int status, String message) throws IOException {
        ...
        response.getWriter().write(String.format(
                "{\"status\": %d, \"error\": \"Unauthorized\", \"message\": \"%s\"}", status, message));
